...
const ignition::math::Vector3d kick_vector_nubot(1,0,0);    // Normalized vector from origin to kicking mechanism in nubot refercence frame.
                                                 // It is subject to nubot model file
const double        goal_x = 9.0;
const double        goal_height = 1.0;
const ignition::math::Vector3d right_goal(goal_x,0.0,0.0);
const double        g = 9.8;
...
void NubotGazebo::UpdateChild()
{
    ...
    if(ball_decay_flag_)
    {
        ignition::math::Vector3d free_ball_vel = football_state_.twist.linear;
...
void NubotGazebo::model_states_CB(const gazebo_msgs::ModelStates::ConstPtr& _msg)
{
  ...
  model_count_ = world_->ModelCount();
...
{  
   ...
   {
        receive_sim_time_ = world_->SimTime();
...
void NubotGazebo::vel_cmd_CB(const nubot_common::VelCmd::ConstPtr& cmd)
{
   this->msgCB_lock_.lock();
    ...
    ignition::math::Vector3d Vy_nubot = Vy_cmd_ * (ignition::math::Vector3d(0,0,1).Cross(kick_vector_world_));
...
void NubotGazebo::kick_ball(int mode, double vel=20.0)
{
   ...
   if(mode == RUN)
   {
        ignition::math::Vector3d vel_vector = kick_vector_planar * vel;
   ...
   else if(mode == FLY)
   {
       ...
       */
        ...
        ignition::math::Vector3d kick_vector(vx*kick_vector_world_.X(), vx*kick_vector_world_.Y(), b*vx);
...
bool NubotGazebo::get_is_hold_ball(void)
{
    ...
    ignition::math::Vector3d norm = nubot_football_vector_;
...
bool NubotGazebo::get_rot_vector(ignition::math::Vector3d target_point_world, double degree_thres)
{
    ...
    static ignition::math::Vector3d last_target_point(10.0,0.0,0.0);
    static common::Time last_current_time = world_->SimTime();
    static double traj_ang;

    // STEP 1: calculate cos angle between nubot_target_vector_ and kick_vector_world_
    ignition::math::Vector3d nubot_target_vector = target_point_world - nubot_state_.pose.position;
    ignition::math::Vector3d nubot_target_vector_norm = nubot_target_vector;
    ...
    {
        if((last_target_point - target_point_world).Length() > 0.5)
        {
            last_current_time = world_->SimTime();
        ...
        double _dt = (world_->SimTime() - last_current_time).Double();
        ...
        ignition::math::Vector3d z_norm;
        if(angle_error_degree > 0)
            z_norm.Set(0.0,0.0,1.0);
        else
            z_norm.Set(0.0,0.0,-1.0);
        ignition::math::Vector3d nubot_rotation_vector = z_norm * (traj_ang * PI/180);
...
bool NubotGazebo::get_trans_vector(ignition::math::Vector3d target_point_world, double metre_thres)
{
    ...
    double nubot_target_vector_length = nubot_target_vector.Length ();
    ...
    else
    {
        ...
        if((last_target_point - target_point_world).Length() > 0.5)                 // CAN TUNE
        {
            last_current_time = world_->SimTime();
        ...
        ignition::math::Vector3d nubot_translation_vector = nubot_target_vector;
...
// There are two ways of ball-dribbling: 1. Set pose; 2. Set tangential velocity 3. Set secant velocity
    ...
    ignition::math::Vector3d       target_pos = nubot_model_->GetWorldPose().pos+ relative_pos;
    ...
    ignition::math::Vector3d actual_nubot_linvel = nubot_state_.twist.linear;
    ignition::math::Vector3d actual_nubot_rotvel = nubot_state_.twist.angular;
    ...
    ignition::math::Vector3d     actual_ball_linear_vel = football_state_.twist.linear;
    actual_ball_linear_vel.Z() = 0;

    nubot_football_vector_.Z() = 0;                                                                // don't point to the air
    ignition::math::Vector3d     perpencular_vel = actual_nubot_rotvel.Cross(nubot_football_vector_);       //debug
    ignition::math::Vector3d     desired_football_vel = actual_nubot_linvel + perpencular_vel;
    desired_football_vel.Z() = 0;
    ignition::math::Vector3d  output_vel = desired_football_vel;

    #if 0       // no need to use PI controller; it works much better without a controller
    /* velocity PID */
    double dt = (world_->SimTime() - last_current_time_).Double();
    double cmd = dribble_pid_.Update( desired_football_vel.Length() - actual_ball_linear_vel.Length(), dt);// error = target - actual
    last_current_time_ = world_->SimTime();
    ...
    #if 0
    double dt = (world_->SimTime() - last_current_time_).Double();
    ignition::math::Vector3d nubot_target_vector_norm = nubot_football_vector_;
    nubot_target_vector_norm.Normalize();
    ...
    set_ball_vel(output_vel, ball_decay_flag_);
        ...
        ignition::math::Vector3d actual_nubot_linvel = nubot_model_->GetWorldLinearVel();
        ignition::math::Vector3d actual_nubot_rotvel = nubot_model_->GetWorldAngularVel();
        ignition::math::Vector3d nubot_football_vector = football_model_->GetWorldPose().pos - nubot_model_->GetWorldPose().pos;
        actual_nubot_linvel.z=0; actual_nubot_rotvel.x=0; actual_nubot_rotvel.y=0;
        static common::Time last_current_time = world_->SimTime();
        const double dt = (world_->SimTime() - last_current_time).Double();
        double theta = actual_nubot_rotvel.Length() * dt;
        math::Matrix3 rota(cos(theta), -sin(theta), 0,
                           sin(theta), cos(theta),  0,
                           0,          0,           1);
        ignition::math::Vector3d b_vector = rota*nubot_football_vector;
        ignition::math::Vector3d c_vector = b_vector - nubot_football_vector;
        ignition::math::Vector3d perpencular_vel = c_vector/dt;
...
{
    static ignition::math::Vector3d target_point;
    static ignition::math::Vector3d nubot_trans_vector;
    static ignition::math::Vector3d nubot_rot_vector;
    ...
    switch(state_)
    {
       ...
       case DRIBBLE_BALL:
       {
           ...
           switch(sub_state_)
           {
               case MOVE_BALL:
               {
                   target_point = ignition::math::Vector3d(4.0, -2.0, 0.0);// goal0_pos - ignition::math::Vector3d(2.0, 1.0, 0);
       ...
       case RESET:
        {
            ...
            if(!get_trans_vector(ignition::math::Vector3d(-4.0,-2.0,0.0), 0.1))
            {
                nubot_locomotion(desired_trans_vector_, ZERO_VECTOR);
            }
            else
...
void NubotGazebo::ball_vel_decay(ignition::math::Vector3d vel, double mu)
{
...
void NubotGazebo::set_ball_vel(ignition::math::Vector3d &vel, bool &ball_decay_flag)
{
